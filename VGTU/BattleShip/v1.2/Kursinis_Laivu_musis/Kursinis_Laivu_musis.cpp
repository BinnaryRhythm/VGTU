...
public:
	void Meniu() {
		...
		while(1) {
			...
			if (cordinates_where_saved_ships_player.size()==0) {
				cout << "lose" << endl;
				break;
			}
		}
	...
	void Move_aim_player(int argX, int argY) {
		...
		if (cordinates_where_saved_ships_enemy.size()!=0&&Map_enemy[y][x] == 3) Move_aim_player(x, y);
	}
	...
	void Move_aim_enemy_first_hit(int argX, int argY) {
		...
		else Move_aim_enemy(cordinates_where_saved_ships_player.size() != 0&&rand() % 7 + 2, rand() % 7 + 2);
	}
